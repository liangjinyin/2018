...
import com.tospur.common.sms.SMSUtils;
...
import com.tospur.modules.sys.dao.MenuDao;
import com.tospur.modules.sys.dao.OfficeDao;
import com.tospur.modules.sys.dao.RoleDao;
import com.tospur.modules.sys.dao.UserDao;
...
import com.tospur.modules.sys.entity.Role;
import com.tospur.modules.sys.entity.User;
import com.tospur.modules.sys.security.SystemAuthorizingRealm.Principal;
...
public class UserUtils {
     ...
     * @return 邮箱地址
     */
    public static  String getEmails(User user){
        user.setLoginName(null);
        user.setName(null);
        List <User> userlist=userDao.findListByOffice(user);
        String emails="";
        for (User email : userlist)
        {
     ...
     * 根据部门信息获取用户
     * @param id
     * @return 发送地址和抄送地址   User对象
     */
    public static  User getEmails(List<Map<String, String>> userlist){
        String copyEmails="xiaohui@tospur.com,";
        User res=new User();
        if(userlist!=null&&userlist.size()>0){
            for (Map<String, String> userName : userlist)
            {
                User  user= userDao.getByLoginName(new User("",userName.get("userName")) );
                if(user!=null && !StringUtils.isEmpty(user.getEmail())){
                    copyEmails+=user.getEmail()+",";
                }
            }
            if(copyEmails.contains(",")){
                copyEmails=copyEmails.substring(0, copyEmails.lastIndexOf(","));
            }
            res.setEmail(copyEmails); 
            res.setCopyEmails(copyEmails); 
        }else{
            return null;
        }
        return res;
     ...
     */
    @Deprecated
    public static  User getEmailsold(List<String> userlist){
        String copyEmails="xiaohui@tospur.com,";
        User res=new User();
        if(userlist!=null&&userlist.size()>0){
     ...
     * @return 发送地址和抄送地址   User对象
     */
    public static  User getEmails(String name){
        User user= userDao.getByLoginName(new User("",name));
        if(user!=null){
            return user;
        }else{
