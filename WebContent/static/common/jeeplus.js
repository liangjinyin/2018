...
//显示详情信息
function  showDetail(obj,name){
    var  content=$(obj).next().val();
    
    var height=400;
    var width=600;
    if(content.length>400 && content.length<= 800){
      height=content.length*1;
      width=content.length*1.2;
    ...
    layer.open({
      ...
      ,content: '<div style="padding:30px;width:100%;height:100%"> '+content+'</div>'
   });    
}
